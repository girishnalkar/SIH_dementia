...
function handleSavePatientPhotoMemory(e) {
  ...
  AudioEngine.speakText(`Photo memory of ${personName} saved! ${caption}`, AppState.currentLanguage);
}

function deletePatientPhotoMemory(photoId) {
  const patientId = AppState.activePatientId;
  if (AppState.patientPhotoMemories[patientId]) {
    AppState.patientPhotoMemories[patientId] = AppState.patientPhotoMemories[patientId].filter(p => p.id !== photoId);
  }
  AudioEngine.playChime('bell');
  renderPatientPersonalNotes();
}

function speakPatientPhotoCaption(photoId) {
  const patientId = AppState.activePatientId;
  const photos = AppState.patientPhotoMemories[patientId] || [];
  const item = photos.find(p => p.id === photoId);
  if (item) {
    AudioEngine.speakText(`${item.personName}. ${item.caption}`, AppState.currentLanguage);
  }
}
